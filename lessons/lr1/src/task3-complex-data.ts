...
function getCourseStats(course: Course, students: Student[]): CourseStats {
    ...
    const allGrades = enrolledStudents
        .flatMap(student => {
            const courseGrades: Grade[] = student.grades?.[course.id] || [];
            return courseGrades;
        })
    ...
    const studentsWithGrades = enrolledStudents.filter(student => {
        const grades: Grade[] | undefined = student.grades?.[course.id];
...
function getTopStudents(students: Student[], courseId: number, limit: number) {
    return students
        ...
        .filter((student): student is Student & { average: number } => student.average !== null)
